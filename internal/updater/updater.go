...
const releaseAPI = "https://api.github.com/repos/yeniklas/memos-tui/releases/latest"
...
func Run(currentVersion string) error {
	...
	if rel.TagName == currentVersion {
		fmt.Printf("memos-tui is already up to date (%s)\n", currentVersion)
		return nil
	}

	assetName := fmt.Sprintf("memos-tui-%s-%s-%s", rel.TagName, runtime.GOOS, runtime.GOARCH)
	...
	probe, err := os.CreateTemp(dir, ".memos-tui-update-*")
	if err != nil {
	...
	fmt.Printf("Update memos-tui %s → %s? [y/N] ", currentVersion, rel.TagName)
	...
	tmp, err := os.CreateTemp(dir, ".memos-tui-update-*")
	if err != nil {
